...
class SWA:
    def __init__(self, model, optimizer, method='back_to_init', 
                 anneal_steps=5, anneal_strategy='linear', alpha=0.6,
                 update_every_n_steps=None):
        self.model = AveragedModel(model, device=model.device)
        self.alpha = alpha
        # make swa_grouped_lr according to given optimizer
        if method == 'back_to_init':
            swa_grouped_lr = list()
            for param_group in optimizer.param_groups:
                swa_grouped_lr.append(param_group['initial_lr']*self.alpha)
        self.anneal_steps = anneal_steps
        if update_every_n_steps is None:
            self.update_every_n_steps = self.anneal_steps
        else:
            self.update_every_n_steps = update_every_n_steps
        self.scheduler = SWALR(optimizer, swa_lr=swa_grouped_lr, anneal_steps=anneal_steps, 
                               anneal_strategy=anneal_strategy, verbose=False)
    ...
    def calc_bn(self, loader):
        # because bert model has no batch normalization layer, so this actually has no effect
        update_bn(loader, self.model)
...
class SWALR(_LRScheduler):
    ...
    Example:
        ...
        >>>        anneal_strategy="linear", anneal_steps=20, swa_lr=0.05)
        >>> swa_start = 160
        >>> for i in range(300):
        >>>      for input, target in loader:
        >>>          optimizer.zero_grad()
    ...
    .. _Averaging Weights Leads to Wider Optima and Better Generalization:
        https://arxiv.org/abs/1803.05407
    """
    def __init__(self, optimizer, swa_lr, anneal_steps=5, anneal_strategy='linear', last_epoch=-1, verbose=False):
        ...
        if not isinstance(anneal_steps, int) or anneal_steps < 0:
            raise ValueError(f"anneal_steps must be equal or greater than 0, got {anneal_steps}")
        self.anneal_steps = anneal_steps
