...
class ExpplotsLosses:
    def __init__(self, dir='./log', loss_keys=['tr_sup', 'tr_sup2', 'tr_unsup'], draw_every_n_steps=20):
        self.dir = dir
        self.losses = {key:[] for key in loss_keys}
        self.loss_keys = loss_keys
        self.steps = []
        self.draw_every_n_steps = draw_every_n_steps
        
    def update(self, losses, step, tag=''):
        for key in self.losses:
            self.losses[key].append(losses[key])
        self.steps.append(step)
        if step%self.draw_every_n_steps==0:
            self._draw(tag)
    
    def _draw(self, tag):
        lineplot(xs=[self.steps for _ in range(len(self.losses))], ys=[self.losses[k] for k in self.losses], title='losses', legend=self.losses.keys(),
                xlabel='steps', ylabel='losses', save_path=self.dir+f'/{tag}_losses_detailed.png', figsiz=(8,6))
