...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, label_ids, match_label):
...
def convert_examples_to_features(examples, max_seq_length, tokenizer):
    ...
    for i, example in enumerate(examples):
        ...
        match_label = example["match_label"]
#         print(len(tokens), len(segment_ids), max_seq_length)
        ...
        feature = InputFeatures(input_ids=input_array, input_mask=mask_array, 
                                segment_ids=segment_array, label_ids=lm_label_array,
...
# train_features = convert_examples_to_features(train_examples, 512, tokenizer)
...
# init epoch1:
train_features = convert_examples_to_features(train_examples, 510, tokenizer)
all_input_ids = torch.tensor([f.input_ids for f in train_features], dtype=torch.long)
all_input_mask = torch.tensor([f.input_mask for f in train_features], dtype=torch.long)
all_segment_ids = torch.tensor([f.segment_ids for f in train_features], dtype=torch.long)
all_label_ids = torch.tensor([f.label_ids for f in train_features], dtype=torch.long)
all_match_label_ids = torch.tensor([int(f.match_label) for f in train_features], dtype=torch.long)
...
if args.do_train:
    ...
    for epoch in range(int(args.num_train_epochs)):
        if epoch % 5 == 0 and epoch != 0:
            ...
            torch.save(model_weights, f'./outputs/pytorch_model_{epoch}-mul.bin')            
        ...
        if epoch % 100 == 0 and epoch != 0:
            ...
            all_match_label_ids = torch.tensor([f.label_id for f in train_features], dtype=torch.long)

        train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_match_label_ids,)
...
#                                                                                   np.mean(train_loss)))
            ...
            input_ids, input_mask, segment_ids, label_ids, match_label = batch
            # masked_lm_loss
            loss = model(input_ids=input_ids, 
                         token_type_ids=segment_ids, 
                         attention_mask=input_mask, 
                         labels=label_ids,
                         next_sentence_label= match_label
                         )[0]
            loss = loss.mean()
