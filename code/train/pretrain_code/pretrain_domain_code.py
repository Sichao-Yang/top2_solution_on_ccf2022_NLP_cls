...
# for pat_name, pat_applicant, pat_summary in tqdm(zip(pydf['pat_name'], pydf['pat_applicant'], pydf['pat_summary'])):
#     pat_summary_len = max_length - len(pat_name) - len(pat_applicant) - 4
#     source_ = "[CLS]" + pat_name + "[SEP]" + pat_applicant + "[SEP]" + pat_summary[: pat_summary_len] +"[SEP]"
#     source.append(source_)
    
# pydf['source'] = source

# pydf = pydf.iloc[: 15000]
tokenizer = BertTokenizer.from_pretrained('/root/autodl-tmp/CCF-小样本/Nezha_pytorch/nezha_model/')
...
model = NeZhaForMaskedLM.from_pretrained('/root/autodl-tmp/CCF-小样本/Nezha_pytorch/nezha_model/')
